...
video = []
train_bvp = []
for idx in range(0,len(allvideo)-5):
    with open('/var/datasets/PURE_webs/'+str(idx)+'-WEBS-'+str(PATCH_SIZE), 'rb') as f:
        (webs,bvp) = pickle.load(f)
        video.append(webs)
        train_bvp.append(bvp)
...
for i in range(0,len(video)):
    video[i],train_bvp[i] = chunks(video[i][:min_len-1], train_bvp[i][:min_len-1],160)


video = torch.cat(video[:])
train_bvp = torch.cat(train_bvp[:])

def save_ckp(model, optimizer, epoch, iteration, path="."):
    checkpoint = {
        'epoch': epoch + 1,
...
def train(model, optimizer, trainloader, epochs, epoch_start=0, iter_start=0):
    for epoch in range(epoch_start, epochs):
        ...
        for i, data in enumerate(trainloader,0):
            ...
            if i >= iter_start: 
                ...
                loss.backward()
                optimizer.step()
                save_ckp(model,optimizer, epoch, i)
...
print(video.shape,train_bvp.shape)
dataset = torch.utils.data.TensorDataset(video,torch.as_tensor(train_bvp))
...
print("epochs: {0}\nStart".format(epochs))
train(model,optimizer,trainloader,epochs,0,0)
